...
const getCurrentQuarter = () => {
  ...
  const selectedValue =
    selectedQuarter && selectedYear ? `${selectedQuarter}|${selectedYear}` : "";
  ...
  return (
    <div className="relative">
      <select
        value={value}
        onChange={onChange}
        className={`w-full appearance-none rounded-md border bg-white px-3 py-2 text-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none ${className}`}
        required={required}
      >
        <option value="" disabled>
          {placeholder}
