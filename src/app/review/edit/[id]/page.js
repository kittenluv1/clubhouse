		...
		const fetchRejectedReview = async () => {
			...
			try {
				...
				if (response.ok) {
					...
					setStartQuarter(review.membership_start_quarter);
					setStartYear(review.membership_start_year.toString());
					setEndQuarter(review.membership_end_quarter);
					setEndYear(review.membership_end_year.toString());
					setTimeCommitment(review.time_commitment_rating);
					setInclusivityRating(review.inclusivity_rating);
					setSocialCommunity(review.social_community_rating);
		...
		<div className="min-h-screen w-full p-10 md:p-12">
			<div className="font-dm-sans mx-auto max-w-7xl">
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					{/* Membership dates */}
					<div className="mt-12 grid max-w-4xl grid-cols-1 gap-5 md:grid-cols-2 pointer-events-none">
						<div>
							<label className="mb-3 block text-lg font-bold">
